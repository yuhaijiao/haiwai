...
$.openLayer = function(p){
	...
	var fs = {
		...
		add_data : function(targetid){					//添加数据到容器，添加事件，初始化下一层次容器
			...
			var data = param.data[pid];								//返回数据变量
		...
		init_style : function() {	//初始化css
			...
			css.push("#headdiv {color:#000;background-color:#ccc;font-size:13px;height:25px;margin:1px;" +aotu+"}");
			...
			var d_css = "{width:"+_width+"px;margin:"+_margin+"px;padding:5px;height:auto;background-color:#fff;float:left;"+aotu+"}";
			css.push("dc "+d_css);
	...
	var _btn_t = $( param.target );

	var xPos = ( $( window ).width() - popupDiv.width() ) * .5;
	var yPos = _btn_t.position().top + _btn_t.outerHeight( true );
