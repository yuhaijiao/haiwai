    ...
    */
    ...
    public function redirectToCas($gateway=false,$renew=false)
    {
        ...
        if (php_sapi_name() === 'cli') {
			echo "<SCRIPT LANGUAGE=\"JavaScript\">parent.window.location.href='$cas_url'</SCRIPT>";
        ...
        if ( $success_elements->item(0)->getElementsByTagName("attributes")->length != 0) {
            ...
            if ($attr_nodes->item(0)->hasChildNodes()) {
                ...
                foreach ($attr_nodes->item(0)->childNodes as $attr_child) {
                    phpCas :: trace("Attribute [".$attr_child->getElementsByTagName('name')->item(0)->nodeValue."] = ".$attr_child->getElementsByTagName('value')->item(0)->nodeValue);
                    $this->_addAttributeToArray($extra_attributes, $attr_child->getElementsByTagName('name')->item(0)->nodeValue, $attr_child->getElementsByTagName('value')->item(0)->nodeValue);
                }
            }
        } else {
